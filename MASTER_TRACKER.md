...
related:
  ...
  - plans/2026-06-14-pillar-a-rigour.md
...
# study-planner-web — Master Tracker

> **What this is.** The one place to see the whole project at a glance: a study-plan web app
> **plus** the M.Tech research that backs its algorithms. Six workstreams, cross-referenced to
> their canonical files. Markers: ✅ done · 🟡 in progress · ☐ not started · 🛑 blocked · ⏳ pending review.
...
**Canonical:** [`prd/PRD-study-tracker-web.md`](prd/PRD-study-tracker-web.md), [`issues/README.md`](issues/README.md), `DEPLOYMENT.md`.
...
**Canonical:** [`plans/2026-06-14-pillar-a-rigour.md`](plans/2026-06-14-pillar-a-rigour.md) + its `VERIFICATION.md`; design notes [`research/doc/2026-06-14-pillar-a-rigour-and-extensions.md`](research/doc/2026-06-14-pillar-a-rigour-and-extensions.md).
...
## 4. Pillar-A A6 — custom calibration + detection  ·  status: ✅ calibration done; detection = honest null (probe only); 🟡 prod integration planned
...
**Production integration (planned 2026-06-20):** [`plans/2026-06-20-enriched-shrink-production-integration/PLAN.md`](plans/2026-06-20-enriched-shrink-production-integration/PLAN.md) (+ `VERIFICATION.md`) ships `enriched_shrink` into `py_progress` → FastAPI `/v1/calibration` → app `useCalibrationState` (server-side, D-01). Reality+frozen priors blended **per-learner**, **gated** on a harness re-validation vs reality-alone (Phase 0 go/no-go, D-02). Detection/projection/scheduling untouched (D-03). 4 phases, ☐ not started.
...
7. **Ship `enriched_shrink` to the app:** execute `plans/2026-06-20-enriched-shrink-production-integration/` — Phase 0 harness-validates the per-learner dual-prior blend (go/no-go vs reality-alone), then promote to `py_progress` → FastAPI `/v1/calibration` → UI `useCalibrationState` (server-side).

...
E2E **can** be run by the agent in this environment. Boot the Vite app dev server (`pnpm --filter @study-tracker/app dev` → http://localhost:5173/study/, needs `COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com`); Supabase env is in `apps/app/.env.local` and Chromium is installed. Run with `pnpm exec playwright test -c e2e/playwright.config.ts <spec> --project=app`.

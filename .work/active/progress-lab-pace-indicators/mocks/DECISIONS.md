...
- **Screen:** Progress Lab modal (`/study/week` -Üí "Open Progress Lab"). The burn-up chart + right-rail "Try a pace" panel. Real components: `apps/app/src/components/ProgressLabModal.tsx`, `BurnUpChart.tsx`, `apps/app/src/roadmap/replan/capacityScenario.ts`.
...
Target: distinguish and label three finishes --- **Planned** (follow the slots -Üí deadline), **GP forecast** (your demonstrated pace -Üí realistic date), **Pace scenario** (+N min/day -Üí adjusted date) --- and re-model the green as an honest forward projection.

Open branches not yet decided:

- Chart line treatment + green-Ü-GP connection + on-chart finish markers
...
- **Date:** 2026-07-19
- **Question:** Should the green scenario line keep the current capacity-fill math (interpolate today's actual -Üí full planned total, reached on the capacity finish date), or be re-modelled as an honest forward projection from today's actual at the boosted daily rate?
- **Applies to baseline state:** initial baseline
- **Mock:** decided in words (no visual variations needed); its *rendering* is Decision 03.
- **Options considered:**
  - **A ---** Forward pace projection: start at today's actual, add `(hoursPerStudyDay √ó 60 + slider delta) √ó demonstrated throughput` per study day until the total is reached; slope = projected pace. **-Üê chosen**
...
- **Date:** 2026-07-19
- **Question:** How should the rail present the three different finish dates --- Planned (follow the slots -Üí deadline), GP forecast (demonstrated pace -Üí realistic date), and Pace scenario (+N min/day)?
...
- **Variations considered:**
  ...
  - **B ---** Plain-language narrative: the same three facts as sentences ("Stick to your slots -Üí Aug 10 --¶ at your pace -Üí -âà Aug 27, 17 days late --¶ +15 min -Üí Aug 18"). **-Üê chosen**
...
- **Date:** 2026-07-19
- **Question:** Lines ran off the right edge --- two finishes (GP forecast and current-pace scenario, -âà Aug 27) fell *past* the old domain end (Aug 22), so the chart felt "un-ending." How do we show a clear end date?
...
- **Variations considered:**
  - **A ---** Goal line + finish flags: a horizontal "Plan complete ¬∑ 55h 40m" line with a dated pin where each trajectory lands. **-Üê chosen**
...
- **Decision 01 (reshape green `.points`): no Replan behavior change**, but shared file -Üí update `capacityScenario.test.ts` point assertions (44-45); keep Replan finish-parity test green (`Replan.test.tsx:184-203`).
- **Fork that WOULD touch Replan** --- making +0 / current-pace reflect *demonstrated* pace (so it equals the GP forecast) changes `finishDate` math:
  - **Option X (recommended):** keep `finishDate` math unchanged; source "current pace / forecast" from the GP curve; +N green keeps existing capacity math. Replan untouched (regression-check only). Parity holds by construction (`H¬∑60+Œ¥` folded into hours = same finish).
  - **Option Y:** re-anchor to demonstrated pace. `finishDate` changes -Üí Replan live finish + `Replan.test.tsx:184-203` + `capacityScenario.test.ts:42-43,65` all update.

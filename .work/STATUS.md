...
- **[APP] Roadmap calendar — `/roadmap` detail + `/roadmaps` history.** 🟡 In progress (impl underway 2026-06-26). Month calendar grid with status-colored session bubbles (done/pending/skipped/unplanned), month nav, hover→modal / dots→day-sheet; new `RoadmapMarkedComplete/Abandoned` events; Edit + Replan stubbed but infra-ready (Python-routed `replanRoadmap` → `POST /v1/roadmap/regenerate`). Visual contract locked (D-14); Playwright visual-walkthrough specs required (D-15, write-only). **Progress:** Step-0 baseline + Phase 1 (`deriveSlotStatuses`) committed `c50ff53`; Phase 2 (read-only current-month calendar) committed `8019007`; Phases 3–4 (month nav + session/day modals) committed `3cb0df6`; all implementer-done and **awaiting Cowork review**. Phases 5–7 not started. **Next:** Cowork review Phases 1–4 vs criteria; then Phase 5 mobile responsive. → [`plans/active/2026-06-26-roadmap-calendar/`](plans/active/2026-06-26-roadmap-calendar/PLAN.md) · log [`VERIFICATION.md`](plans/active/2026-06-26-roadmap-calendar/VERIFICATION.md)

...
- **[APP] Roadmaps dashboard — post-ship bug fixes.** ☐ Plan drafted, awaiting implementation. Three defects from manual testing: **(P0)** new-roadmap onboarding can't advance — `?new=1` dropped by every intra-onboarding `navigate()`, so `OnboardingGate` redirects returning users to `/home` on Continue (also mis-branches `Step3Preview`); fix = preserve search param across step navs + latch mode in the gate (Phase A). **(P1)** historical-roadmap detail has no close/URL — opens inline via component state; fix = view at `/roadmap?roadmap=<createdAt>` read-only with back link (Phase B). **(P2)** "bad layout" — blocked on OQ-01 (needs Rohit to specify). Phases A & B grounded with exact code; VERIFICATION pre-filled. → [`plans/active/2026-06-27-roadmaps-dashboard-fixes/`](plans/active/2026-06-27-roadmaps-dashboard-fixes/PLAN.md) · log [`VERIFICATION.md`](plans/active/2026-06-27-roadmaps-dashboard-fixes/VERIFICATION.md)

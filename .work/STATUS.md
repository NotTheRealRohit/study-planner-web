...
- **[APP] Roadmap calendar — `/roadmap` detail + `/roadmaps` history.** ☐ Planned (grilled 2026-06-26). Month calendar grid with status-colored session bubbles (done/pending/skipped/unplanned), month nav, hover→modal (desktop) / dots→day-sheet (mobile); new `RoadmapMarkedComplete/Abandoned` events; Edit + Replan stubbed but infra-ready, incl. a Python-routed `replanRoadmap` interface (`POST /v1/roadmap/regenerate`). Visual contract locked (D-14: recessed current-week band, warm today wash, rust only on skipped); Playwright visual-walkthrough specs required (D-15, write-only). 7 phases, not yet implemented. → [`plans/active/2026-06-26-roadmap-calendar/`](plans/active/2026-06-26-roadmap-calendar/PLAN.md)

...
- **[APP] Material-session post-ship UI bug fixes.** 🟡 Phases 1-4 implemented in `0268f20` and `3c8d869`, awaiting review.
  Phase 1 removed the duplicate `Edit & add` roadmap-hero link; Phase 2 centered shared booking sheets; Phase 3 added the compact empty-day DaySheet add-session path; Phase 4 added study-day tint, headers, legends, and onboarding booked-chip legibility.
  Verification passed: Phase 1/2 greps, `Roadmaps` test, app typecheck, and sheet geometry checks; Phase 3/4 greps, `pnpm --filter @study-tracker/app typecheck`, and `pnpm --filter @study-tracker/app test -- CalendarCell RoadmapCalendar Step3Preview calendarModel` (58 files, 515 tests).
  E2E cases for BUG-2 and BUG-4 are authored but not run per plan.
  No event-model, intelligence-math, or Python changes.
  4d back-to-`/roadmaps` exit remains deferred (OQ-01).
  **Next:** reviewer pass on Phases 1-4, then execute Phase 5 burn-up chart fix.
  → plan [`plans/active/2026-07-02-material-session-ui-bugs/PLAN.md`](plans/active/2026-07-02-material-session-ui-bugs/PLAN.md) · verification [`VERIFICATION.md`](plans/active/2026-07-02-material-session-ui-bugs/VERIFICATION.md) · scratchpad [`SCRATCHPAD.md`](plans/active/2026-07-02-material-session-ui-bugs/SCRATCHPAD.md) · mock [`study-day-indicator.html`](plans/active/2026-07-02-material-session-ui-bugs/mocks/proposed/study-day-indicator.html)

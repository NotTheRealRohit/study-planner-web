...
env COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com pnpm --filter progress test
# 72 passed

env COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com pnpm --filter roadmap-engine test
# 30 passed
```

Why the env var matters: on this machine plain `pnpm` can fail because Corepack attempts the public npm registry and gets `403`. The repo-local workaround is documented in `.claude/rules/pnpm-build-registry.md`; use `COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com` for pnpm verification.

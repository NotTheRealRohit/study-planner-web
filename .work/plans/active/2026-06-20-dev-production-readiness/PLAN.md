...
**Plan status:** Draft
**Upstream:** builds on [`../2026-06-20-enriched-shrink-production-integration/PLAN.md`](../2026-06-20-enriched-shrink-production-integration/PLAN.md) (server-side calibration shipped + Cowork-verified). Resolves its OQ-02 (offline/caching) and the dev-side of OQ-03 (auth/CORS). Production *hosting* remains deferred (apps are not deployed yet).
...
**Status:** 🔁 Changes requested (committed 5cabf41; Cowork review 2026-06-25) — typed-error contract leaks on timeout/network exhaustion (`normalizedError` returns raw `AbortError`/`TypeError`); see VERIFICATION.md Phase 2.
**Depends on:** Phase 1

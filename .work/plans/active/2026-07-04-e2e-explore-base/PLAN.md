...
- Run: `env COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com pnpm exec playwright test -c e2e/playwright.config.ts e2e/support/__smoke__/base.spec.ts --project=app --reporter=list`

#### Verification (DONE — run after implementation)

```bash
env COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com \
...
- This *is* the test. Run: `env COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com pnpm exec playwright test -c e2e/playwright.config.ts e2e/app-explore.spec.ts --project=app --reporter=list`
...
- Run: `env COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com pnpm exec playwright test -c e2e/playwright.config.ts e2e/session-log.spec.ts --project=app --reporter=list`
...
1. **Create `e2e/README.md`** covering: the `e2e/support/` layout; how to write a new spec (`import { test, expect } from './support/fixtures'`, use `signedInPage`, seed with `seedEvents`, select via `selectors.ts`); the run command (`env COREPACK_NPM_REGISTRY=https://npm.dev.payment-providerinc.com pnpm exec playwright test -c e2e/playwright.config.ts <spec> --project=app`); hermetic-vs-live modes; and the **anti-patterns** the base forbids (`waitForTimeout`, inlined `signIn`/`createTestUser`, hardcoded button labels, failure-swallowing `try/catch`).

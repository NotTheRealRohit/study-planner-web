...
9. ✅ #19 Grading → mastery signal — resolved 2026-08-04. **Governing principle (learner directive): nothing pinned by fiat — every algorithm + constant is selected/tuned on the kt-bench bake-off; #19 fixes the interface, search space, metrics, tuning protocol.** #19 owns the observation→mastery **interface + a Bayesian baseline**; #20 swaps full BKT behind it. Mastery keys on **`materialId`** (#11/#15; skillTag refinement axis reserved for #20). Pluggable estimator `estimate(obs)→{p∈[0,1],confidence,n}` with 3 candidates (EWMA / Beta-Bernoulli / Gaussian-clamped), **default+constants chosen by a `research/kt-bench` bake-off** (ECE / AUC-accuracy / cold-start-`k`, alongside pyBKT). Compute = server-side stateless **`/v1/mastery`** (`py-progress` authoritative, `calibrationCache` precedent; **no server store, no migration**), TS mirror for parity/offline-read. Pure projection — **no `MasteryUpdated`** (reserved). Mastery-estimator bake-off folded into **#23**. **Unblocks #20.**
10. ☐ #20 KT model & adaptive-difficulty loop — **UNBLOCKED by #19 (2026-08-04)** → frontier. Swaps full BKT (pyBKT) behind #19's `/v1/mastery` interface; owns `observedDifficulty` + difficulty-weighting + adaptive selection loop + online cadence + roadmap signal (#26). Baseline-vs-BKT comparison tracked in #23.
...
15. ☐ #23 Generation-quality evaluation harness — blocked by #18 (impl) + #20 (difficulty-calibration + baseline-vs-BKT) + #19 (mastery bake-off — satisfied); written triggers. **#19 folded in a 4th deliverable (2026-08-04): mastery-estimator bake-off** — sweep the 3 estimators + constants on kt-bench, pin winners = `/v1/mastery` defaults (baseline sweep does not wait on #20).
...
| [#19](https://github.com/NotTheRealRohit/study-planner-web/issues/19) | Grading → mastery signal mapping | grilling | 3 | ✅ closed — interface + Bayesian baseline (#20 swaps BKT behind it); mastery keys on `materialId` (skillTag axis reserved); 3-candidate pluggable estimator, default+constants chosen by a kt-bench bake-off (nothing pinned by fiat); server-side stateless `/v1/mastery` (`py-progress`, no store/no migration); pure projection (`MasteryUpdated` reserved); bake-off → #23; unblocks #20 |
| [#20](https://github.com/NotTheRealRohit/study-planner-web/issues/20) | KT model & adaptive-difficulty loop | grilling | 3 | ✅ **UNBLOCKED by #19** → frontier; swaps full BKT/pyBKT behind `/v1/mastery`; owns `observedDifficulty` (#23 calibration target) + adaptive loop + roadmap signal (#26) |
...
| [#23](https://github.com/NotTheRealRohit/study-planner-web/issues/23) | Generation-quality evaluation harness (groundedness / difficulty-calibration / dedup / mastery-estimator bake-off) | task | 3 | #18 (groundedness+dedup) · #20 (difficulty-calibration + baseline-vs-BKT) · #19 (mastery bake-off — satisfied); written triggers; **#19 folded in a 4th deliverable 2026-08-04** |

...
8. ☐ #13 Vector store, embeddings & async jobs — **unblocked by #12** (base set: `content_chunks.embedding halfvec(768)` HNSW cosine; chunk→Ingestion worker, embed→Intelligence worker; job mechanism ✅ #22=pgmq). Remaining: chunk params (open-notebook default 400-token/15% overlap), embed batching vs Gemini free-tier quota, RAG retrieval query (top-k/threshold/per-`material_id`), re-embed on replace. Trio unblocks #18.
...
| [#13](https://github.com/NotTheRealRohit/study-planner-web/issues/13) | Vector store, embeddings & async job mechanism | grilling | 2 | ☐ open — **unblocked by #12** (base set; job mech ✅ #22) |

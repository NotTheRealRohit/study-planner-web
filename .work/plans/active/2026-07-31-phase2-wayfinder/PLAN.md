...
7. ☐ #12 Content ingestion & storage — **unblocked by #22** (spine set). Decided so far: sources = all 4 (upload/URL/paste/YouTube); deterministic extraction (no LLM), LLM topics/summary via Intelligence Service bound to `material_id`; separate Ingestion Service confirmed. Resume: concrete `materials`/content table shape + RLS, raw-file bucket, ingestion-status model, `MaterialAdded`↔content-material reconciliation.
8. ☐ #13 Vector store, embeddings & async jobs — consumes #12; **job mechanism ✅ decided in #22 (pgmq; embed = Intelligence worker)**. Trio unblocks #18.
...
| [#12](https://github.com/NotTheRealRohit/study-planner-web/issues/12) | Content ingestion & storage design | grilling | 2 | ☐ open — **unblocked by #22** |
| [#13](https://github.com/NotTheRealRohit/study-planner-web/issues/13) | Vector store, embeddings & async job mechanism | grilling | 2 | ☐ open — blocked by #12 (job mech ✅ #22) |
